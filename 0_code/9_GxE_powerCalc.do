*DO GxE POWER CALCULATIONS FOR EFFECT SIZE & SAMPLE SIZE AT THE SAME TIME
...
local repl = 1000 //1000

*SET SAMPLE SIZES: there will be one line in the graph for each sample size
//mat samplesizes = (1000 \ 2000 \ 3000 \ 5000 \ 10000)

*here we have 4 different regressions whose power we want to test, and we know the sample size for each
...
mat    beta_E = (0.90 \ 0.60 \ 0.35 \ 0.20 \ 0.13)     // effect of E: taken from Crawford et al. (2010); one for each key-stage; has to be the same length as sample sizes
...
scalar sd_G = 1.00    // standard dev of the genes, equal to 1 for PGI
    ...
    forvalues jGrid = 1/`betas' {
		...
		forvalues iRepl = 1/`repl' {
			...
			*gen E = rnormal(0,sd_E)      // use this for continuous E
			gen E = (runiform()>share_E)  // use this for dichotomous E; equal to 1 if random uniform distribution is bigger than share of treated
...
*GRAPHS FOR THE T-TEST, NOT THE LR TEST
capture scalar beta_E = beta_E[1,1] 

local beta_0 = beta_0
local beta_G = beta_G
local beta_E = beta_E
local sd_error = sd_error
local  equation "Y = `beta_0' + `beta_G' G + `beta_E' E + beta_GxE (GxE) + rnormal(0,`sd_error')"
di "`equation'"

if `ss'==1 {
twoway (line overall_power1power1 overall_power1beta) ///
       , ytitle("Power") xtitle("Coefficient of the GxE  interaction term") ///
         note("Note: Number of replications `repl'; DGP: `equation'", size(small)) ///
